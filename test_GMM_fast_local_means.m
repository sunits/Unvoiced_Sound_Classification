function []=test_GMM_fast_local_means(full_path, ... %
...
for channel=1:number_of_channels
   ...
   for class_index=1:number_of_classes
       load([gmm_path(class_index).path  'Channel',num2str(channel)]);
       
       %f_mean  and f_std loads from the stored model
        temp_features=(features_for_training-repmat(f_mean,size(features_for_training,1),1))./repmat(f_std,size(features_for_training,1),1);
        temp_prob(:,class_index)=pdf(model,temp_features);
   end
...
% [phTruth,phClassification,phAccuracy]=frameWindowing(all_boundary, ... % Information about number of frames per phonemes
%                                         [result(ph_index).predicted], ... % frame level classification result
%                                         [result(ph_index).truth], ... % frame level truth values
%                                         number_of_classes); % Total number of classes
...
save([prediction_save_path ],'prob_model');
