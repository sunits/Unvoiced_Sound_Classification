function []=test_GMM_moment_channel_find_best_channels(full_path, ... %
...
%% find which channels gives better accuracy

% parameters are: class_prob,true_index 
...
result(ph_index).vote_predicted=voting(class_prob,length(ph_to_train),prob_model_path,phonemes_to_compare,channels_to_consider);
...
% fprintf('Accuracy for ph:%d : %f - ph A \n',ph_index,acc);
fprintf('\n ph: %d voting Acc =%d : With voting + windowin= %d\n',ph_index,acc,phAccuracy);

clear all_*;

end
save([prediction_save_path 'result_20_32.mat'] ,'result');

confusion_matrix(phTruth,phClassification,{'sh','s','f','th'});
