...
gmm_path=cell2mat([full_path, [ph_to_train(ph_index)], '/gmm_modPower_scaled/']);
mkdir(gmm_path);
...
%         all_cross_corr(counter).cc=feature;        
        all_cross_corr(counter).cc=temp(:,:,temp_index);
    end
end
% Vectorize the feature
 all_cross_corr=scaling_factor*[all_cross_corr(:).cc];

for channel=1:number_of_channels
   features_for_training= all_cross_corr(channel,:);   
   number_of_training_samples=length(features_for_training)/(mod_filter_size_len);
   features_for_training=reshape(features_for_training,number_of_training_samples,mod_filter_size_len);
