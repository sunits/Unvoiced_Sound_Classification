function []=test_GMM_moment_fast_full_ph(full_path, ... %
			...
			num_of_moments, ... % Not all channels give differentiability. So which ones are to be included
...
number_of_classes=length(phonemes_to_compare);
...
class_prob=zeros(size(all_m1,2),sum(channels_to_consider));
counter=0;
...
result(ph_index).vote_predicted=voting(class_prob,length(phonemes_to_compare),prob_model_path,phonemes_to_compare,channels_to_consider);
...
confusion_matrix(truth,predicted,ph_to_train(phonemes_to_compare));


save([prediction_save_path 'results_11_32.mat'] ,'result');
